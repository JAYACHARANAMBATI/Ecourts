...
def solve_and_load_case():
    ...
    try:
        ...
        # --- CAPTCHA and Submit ---
        API_KEY = "AIzaSyC-hS6f2Tzqmew_PMJ9qIbUUcDK1xGjBtw"  # Use your Gemini API key

        for attempt in range(1, 11):
            print(f"\n🔁 Attempt {attempt}: Solving CAPTCHA automatically...")
            captcha_img = wait.until(EC.presence_of_element_located((By.ID, "captcha_image")))
            captcha_img_path = f"captcha_attempt_{attempt}.png"
            captcha_img.screenshot(captcha_img_path)
            print(f"📸 CAPTCHA saved: {captcha_img_path}")

            captcha_text = solve_captcha_with_gemini(captcha_img_path, API_KEY)
            print(f"🔍 CAPTCHA Text (Gemini): {captcha_text}")
